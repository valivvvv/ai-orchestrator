...
**Phase 2 results & deviations (2026-06-23):**
- **System prompt sizing:** Romanian is ~3 chars/token, so the prompt needed two expansion passes to clear the 4096 floor. Final `orchestrator_system.yaml` = 13,875 chars → **4,514 tokens** (Haiku 4.5 `count_tokens`); cached prefix observed as **4,508 tokens**. Comfortable ~400-token margin.
- **`_build_messages` helper** added to `Orchestrator` (rather than inlining the system message in each node) — single gate point for `getattr(self.llm, "cache_system", False)`; both `node_evaluate` and `node_answer` route through it.
