...
### Phase 2 — RAG Agent + Orchestrator (L5 retry/refine + L6 supervisor-over-worker) — ✅ DONE

> **Deviation 1 (recorded per §0):** `node_search` builds its `SearchResultItem`s *after* the `with transaction()` block closes, which raises `DetachedInstanceError` (committed ORM instances are expired+detached). Fixed at the source: `database.py` `SessionLocal` now uses `expire_on_commit=False` so committed instances keep their loaded attributes. `node_search` itself is untouched. (Phase 1 never hit this — `node_execute_sql` uses `result.mappings()`, not ORM objects.)
>
> **Deviation 2 (recorded per §0, anticipated by pitfall #9):** `node_call_rag` read `rag_result.result` (attribute access), but `RAGAgent.run` returns `graph.invoke(...)`, which is a `dict` in the installed LangGraph (matching `main.py`'s `result['status']` dict access). Changed to `rag_result["result"]`. The return type comes from LangGraph, so there is no stub-only fix.
...
### Phase 4 — Analyst Agent (L6 supervisor: plan → workers → synthesize) — ✅ DONE
